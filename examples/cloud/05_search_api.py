...
This example demonstrates the OpenBrowser Search API (BETA):
...
if not API_KEY:
	raise ValueError(
		'Please set BROWSER_USE_API_KEY environment variable. You can also create an API key at '
	)

BASE_URL = os.getenv('BROWSER_USE_BASE_URL', 'https://api.browser-use.com/api/v1')
TIMEOUT = int(os.getenv('OPENBROWSER_TIMEOUT', '30'))
...
async def demo_deep_website_analysis():
	...
	demo_start = time.time()
	url = 'https://openbrowser.github.io/docs'
	query = 'OpenBrowser features, pricing, and API capabilities'
...
async def demo_real_time_vs_cached():
	...
	print('🔄 OpenBrowser Search API benefits:')
