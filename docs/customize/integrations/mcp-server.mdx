...
The MCP server exposes a single tool that runs Python code in a persistent namespace with browser automation functions. Functions are async unless noted; use `await`.
...
| **Downloads** | `download_file(url, filename)`, `list_downloads()` (sync, no `await`) |
