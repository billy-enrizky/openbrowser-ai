...
print(state.title)
print(f'{len(state.selector_map)} interactive elements')
"
```
...
```
  CLI (-c flag)           Daemon Server           Browser
  +-----------+       +------------------+     +----------+
  | openbrowser -c |-->| Unix socket      |---->| Chrome   |
  | "code..."  |   | ~/.openbrowser/  |     | (CDP)    |
  +-----------+   | daemon.sock      |     +----------+
                  |                  |
                  | CodeExecutor     |
                  | (persistent      |
...
| **Idle timeout** | 10 minutes (configurable) | Session-based |
...
- Use `openbrowser daemon status` to check if the daemon is running
