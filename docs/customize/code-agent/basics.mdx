...
- `click_xy(x, y, button='left', click_count=1)` - Click a no-index canvas, PDF, embed, or native-rendered target
- `hover_xy(x, y)` - Move over a no-index canvas, PDF, embed, or native-rendered target
- `input(index, text)` - Type text into an input
- `scroll(down, pages)` - Scroll the page
- `scroll_xy(x, y, delta_x=0, delta_y=0)` - Wheel-scroll a no-index surface
...
Prefer indexed actions whenever an element index exists. The `*_xy` coordinates are CSS viewport pixels, not screenshot pixels. If a screenshot uses a `devicePixelRatio` other than 1, divide its coordinates by `window.devicePixelRatio` first.
