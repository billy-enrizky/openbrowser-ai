...
## Token Usage: What Gets Measured

There are two distinct token metrics. They tell very different stories.

### Metric 1: Bedrock API Tokens (What You Actually Pay For)

This is the real LLM cost -- `inputTokens` + `outputTokens` from the Bedrock Converse API `usage` field. Includes system prompt, tool schemas, conversation history, and tool results. This is what shows up on your AWS bill.

From a single benchmark run across 6 tasks (Claude Sonnet 4.6, Bedrock Converse API):
...
| **Total** | **150,248** | **310,856** | **49,423** |
| **Ratio vs OpenBrowser** | 3.0x more | 6.3x more | 1x (baseline) |

OpenBrowser uses **3x fewer tokens** than Playwright and **6.3x fewer** than Chrome DevTools.

### Metric 2: MCP Response Size (Tool Output Only)

This measures the character count of text returned from MCP tool calls (chars / 4 as token estimate). This is a subset of the Bedrock API input tokens -- it does not include system prompts, tool schemas, conversation history, or LLM output.

5-step workflow on Wikipedia (JSON-RPC stdio, same operations):
...
| **Response size ratio** | 877x more | 476x more | 1x (baseline) |

The response size difference is real and large, but **response size is not the same as total LLM cost**. The LLM also pays for system prompt, tool schemas (Playwright has 22 tools, Chrome DevTools has 26, OpenBrowser has 1), and accumulated conversation history.
...
**OpenBrowser MCP**: The LLM writes Python code that processes browser state inside the server-side runtime. Only extracted results come back. The LLM relies on general web knowledge to write speculative code, and gets compact feedback (~30-1,000 chars per call) rather than full page dumps.
