...
Sandboxes are the **easiest way to run OpenBrowser in production**. We handle agents, browsers, persistence, auth, cookies, and LLMs. It's also the **fastest way to deploy** - the agent runs right next to the browser, so latency is minimal.
...
```python
from openbrowser import Browser, sandbox, ChatBrowserUse
