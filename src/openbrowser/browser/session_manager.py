"""Event-driven CDP session management following browser-use pattern."""
...
class SessionManager:
    ...
    def __init__(self, browser_session: 'BrowserSession'):
        ...
        Args:
            browser_session: The browser session to manage
        """
        ...
        def on_attached(event: AttachedToTargetEvent, session_id: SessionID | None = None):
            ...
            async def _enable_auto_attach():
                try:
                    ...
                    self.logger.debug(
                        f'[SessionManager] Auto-attach enabled for {target_type} session {event_session_id[:8]}...'
                    )
    ...
    async def _handle_target_attached(self, event: AttachedToTargetEvent) -> None:
        ...
        async with self._lock:
            ...
            if target_id not in self.browser_session._cdp_session_pool:
                from src.openbrowser.browser.session import CDPSession

                # Guard against race condition during browser shutdown/restart
                if self.browser_session._cdp_client_root is None:
                    self.logger.debug(
                        f'[SessionManager] Skipping session creation for {target_id[:8]}... - CDP client not available'
                    )
                    return
    ...
    async def _handle_target_detached(self, event: DetachedFromTargetEvent) -> None:
        ...
        async with self._lock:
            ...
            if target_id in self._target_sessions:
                ...
                if remaining_sessions == 0:
                    self.logger.debug(
                        f'[SessionManager] No sessions remain for target {target_id[:8]}..., removing from pool'
                    )
                    ...
                    agent_focus_lost = (
                        ...
                        and self.browser_session.agent_focus.target_id == target_id
                    )
        ...
        # Dispatch TabClosedEvent only for page/tab targets that are fully removed
        if target_fully_removed:
            if target_type in ('page', 'tab'):
                from src.openbrowser.browser.events import TabClosedEvent
                ...
                self.logger.debug(
                    f'[SessionManager] Target {target_id[:8]}... fully removed (type={target_type}) - '
                    f'not dispatching TabClosedEvent'
                )
    ...
    async def _recover_agent_focus(self, crashed_target_id: TargetID) -> None:
        ...
        async with self._recovery_lock:
            ...
            if (
                self.browser_session.agent_focus
                and self.browser_session.agent_focus.target_id != crashed_target_id
            ):
                self.logger.debug(
        ...
        try:
            ...
            if new_session:
                ...
                self.browser_session.event_bus.dispatch(
                    AgentFocusChangedEvent(target_id=new_target_id, url=new_session.url)
                )
                return

            # Recovery failed - create emergency fallback tab
            self.logger.error(
                f'[SessionManager] Failed to get session for {new_target_id[:8]}... after 2s, '
                f'creating emergency fallback tab'
            )
            ...
            for _ in range(20):
                ...
                if fallback_session:
                    ...
                    self.logger.warning(
                        f'[SessionManager] Agent focus set to emergency fallback: {fallback_target_id[:8]}...'
                    )

                    from src.openbrowser.browser.events import AgentFocusChangedEvent, TabCreatedEvent
            ...
            self.logger.critical(
                '[SessionManager] CRITICAL: Failed to recover agent_focus even with fallback! '
                'Agent may be in broken state.'
            )
