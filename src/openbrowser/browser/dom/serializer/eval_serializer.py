...
class DOMEvalSerializer:
    ...
    def serialize_tree(node: SimplifiedNode | None, include_attributes: list[str], depth: int = 0) -> str:
        ...
        if node.original_node.node_type == NodeType.ELEMENT_NODE:
            ...
            # Default to visible if snapshot_node is None (no visibility data available)
            is_visible = node.original_node.is_visible if node.original_node.is_visible is not None else True
            ...
            # Add scroll info if element is scrollable (if available)
            # Note: This requires EnhancedDOMTreeNode to have should_show_scroll_info and get_scroll_info_text
            # For now, we'll skip this if not available
            if hasattr(node.original_node, 'should_show_scroll_info') and node.original_node.should_show_scroll_info:
                if hasattr(node.original_node, 'get_scroll_info_text'):
            ...
            for attr in EVAL_KEY_ATTRIBUTES:
                if attr in node.attributes:
                    ...
                    if attr == 'class':
                        ...
                        # For href, cap at 80 chars to save space
    ...
    def _serialize_iframe(node: SimplifiedNode, include_attributes: list[str], depth: int) -> str:
        ...
        # Add scroll info for iframe content (if available)
        if hasattr(node.original_node, 'should_show_scroll_info') and node.original_node.should_show_scroll_info:
            if hasattr(node.original_node, 'get_scroll_info_text'):
                scroll_text = node.original_node.get_scroll_info_text()
                if scroll_text:
    ...
    def _serialize_document_node(
        ...
        if dom_node.node_type == NodeType.ELEMENT_NODE:
            ...
            # For regular content, default to visible when no visibility data
            if is_iframe_content:
                ...
                is_visible = dom_node.is_visible if dom_node.is_visible is not None else True
            else:
                # Default to visible when no visibility data available
                is_visible = dom_node.is_visible if dom_node.is_visible is not None else True
