"""
OAuth2 Device Authorization Grant flow client for openbrowser.
"""
...
def get_or_create_device_id() -> str:
	...
	device_id_path = CONFIG.OPENBROWSER_CONFIG_DIR / 'device_id'
	...
	CONFIG.OPENBROWSER_CONFIG_DIR.mkdir(parents=True, exist_ok=True)
...
class CloudAuthConfig(BaseModel):
	...
	def load_from_file(cls) -> 'CloudAuthConfig':
		...
		config_path = CONFIG.OPENBROWSER_CONFIG_DIR / 'cloud_auth.json'
		...
		try:
			...
			if token_data and token_data.get('access_token'):
				...
				if show_instructions:
					logger.debug('Authentication successful! Cloud sync is now enabled with your openbrowser account.')
