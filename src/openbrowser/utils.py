...
def get_openbrowser_version() -> str:
	...
	try:
		...
		version = str(get_version('openbrowser-ai'))
		os.environ['LIBRARY_VERSION'] = version
		return version

	except Exception as e:
		logger.debug(f'Error detecting openbrowser version: {type(e).__name__}: {e}')
		return 'unknown'


async def check_latest_openbrowser_version() -> str | None:
	"""Check the latest version of openbrowser-ai from PyPI asynchronously.
	...
	try:
		async with httpx.AsyncClient(timeout=3.0) as client:
			response = await client.get('https://pypi.org/pypi/openbrowser-ai/json')
